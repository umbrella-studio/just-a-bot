...
async function startA17() {
  ...
  A17.ws.on("CB:call", async (json) => {
    ...
    if (json.content[0].tag == "offer") {
      ...
      A17.sendMessage(
        callerId,
        { text: `Baka! You will be blocked automatically for calling me!` },
        { quoted: pa7rick }
      );
      await sleep(8000);
  ...
  A17.ev.on("groups.update", async (pea) => {
    ...
    if (pea[0].announce == true) {
      ...
      A17.sendMessage(m.chat, {
        image: wm_fatih,
        caption: "Grop has been *Closed!* Only *Admins* can send Messages!",
      });
    } else if (pea[0].announce == false) {
      // A17.send5ButImg(pea[0].id, `Grop has been *Opened!* Now *Everyone* can send Messages!`, `A17 Bot`, wm_fatih, [])
      A17.sendMessage(m.chat, {
        image: wm_fatih,
        caption: "Grop has been *Opened!* Now *Everyone* can send Messages!",
      });
